<template>
  <div class="studentOp">
            ...
            <el-table-column
              ...
              <template slot="header" slot-scope="scope" >
                <el-select v-model="select" placeholder="请选择查询" width="80">
                      ...
                      <el-option label="学生姓名" value="sname"></el-option>
                      <el-option label="课程号" value="cno"></el-option>
                      <el-option label="课程名称" value="cname"></el-option>
                      <el-option label="上课教师姓名" value="tname"></el-option>
                      <el-option label="系号" value="sdept"></el-option>
                      <!-- <el-option label="总评成绩" value="total_score"></el-option> -->
                </el-select>
                <el-input
                  name='search'
                  width="80"
      ...
      <el-dialog :append-to-body="true" title="编辑成绩" :visible.sync="dialogFormVisible" class="thisblack-bgc">
          <el-form :model="singalInfo" class="flexForm">
              ...
              <el-form-item label="课程号" :label-width="formLabelWidth">
                  <el-input v-model="singalInfo.cno" :placeholder="singalInfo.sname" autocomplete="off" class="popWinInput"></el-input>
              </el-form-item>
              <el-form-item label="平时成绩" :label-width="formLabelWidth">
                  <el-input v-model="singalInfo.often_score" autocomplete="off" class="popWinInput"></el-input>
              </el-form-item>
              <el-form-item label="考试成绩" :label-width="formLabelWidth">
                  <el-input v-model="singalInfo.test_score" autocomplete="off" class="popWinInput"></el-input>
              </el-form-item>
              <el-form-item label="总评成绩" :label-width="formLabelWidth">
                  <el-input v-model="singalInfo.total_score" autocomplete="off" class="popWinInput"></el-input>
              </el-form-item>
          </el-form>
...
import {getGradeInfo,deleteInfo,modifyInfo,findSpecialInfo} from '@/api/grade.js'
...
export default {
  ...
  async created(){
    //一开始获得表格数据
    await this.getAgainTableData()
    // console.log(this.tableDataNew.cname)

  },
  methods: {
      //（重新）获取表格数据并将事件进行格式化
      async getAgainTableData(){
        const {data:res} = await getGradeInfo()
      ...
      async searchData(){
        ...
        console.log(res)
        this.tableDataNew = await res.result.result
        // this[(this.select==='cname'?'isShowCname':'')]()
        // this.isShowCname(this.select)
        
        // this.timeFilter()
                this.$nextTick(()=>{
          this.$refs.tableInfo.doLayout()
        })
      ...
      isShowCname(select){
        ...
        if (this.tableDataNew[select] == undefined) return false
        // console.log( Object.hasOwn(this.tableDataNew[0], 'cname'))
        return Object.hasOwn(this.tableDataNew[0], select)
      },
      // isShowSname(){
      //   return Object.hasOwn(this.tableDataNew[0], 'sname')
      // },
      //提交数据
      async makesureBtn(){
          const id = this.uid
          const id2 = this.uid2
          const {data:res} = await modifyInfo({id,id2,...this.singalInfo})
          console.log(res)
      },
      //删除数据
      async deleInfo(row){
         await deleteInfo(row.sno,row.cno)
         //重新获得数据
